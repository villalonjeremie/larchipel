...
if (!$uncodeblock_found) :
	...
	$output.= '<div data-parent="true" class="' . esc_attr(trim($css_class)) . '"' . $row_name . $row_style . $el_id . '>';
	if ($unlock_row === 'yes') {
	...
	if ( $is_header === 'yes' || ! function_exists('vc_is_page_editable') || ! vc_is_page_editable()  ) {
		echo '<script id="'.esc_attr($script_id).'" data-row="'.esc_attr($script_id).'" type="text/javascript" class="vc_controls">UNCODE.initRow(document.getElementById("'.$script_id.'"));</script>';
	}
	$output = '';
	if (!$with_slider) {
		$output.= '</div>';
